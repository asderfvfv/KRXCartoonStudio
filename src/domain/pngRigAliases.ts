/**
 * Map loose PNG filenames (RU/EN slang) → rig slots.
 * Examples: голова.png, Башка.PNG, head, рука-левая, arm_right, тело
 */
...
/** Canonical aliases per slot (normalized stems). */
...
function aliasHitsStem(stem: string, alias: string): boolean {
  ...
  // single-token alias contained as whole token only
  if (!alias.includes("-") && stemTokens.includes(alias)) return true;
  // голова → головашка, башк → башка
...
/** Best matching slot for a filename stem, or null. */
export function matchPngStemToRigSlot(fileName: string): RigPngSlot | null {
  ...
  for (const slot of Object.keys(RIG_SLOT_ALIASES) as RigPngSlot[]) {
    for (const alias of RIG_SLOT_ALIASES[slot]) {
      ...
      // Prefer longer alias + side match
...
/**
 * Pick one file for a slot. Prefer exact/side-specific names.
 * `used` paths are skipped so left/right don't steal the same file when alternatives exist.
 */
export function pickPngForRigSlot(
  ...
  for (const file of files) {
    ...
    if (matched !== slot) {
      // Shared generic names (eye/arm/leg/hand) may map to *-left first;
      // allow generic right fallback only when no side hint conflicts.
...
export function listAcceptedAliasesForSlot(slot: RigPngSlot): string[] {
  return [...RIG_SLOT_ALIASES[slot]];
}
